...
function StoryCard({ story, onReadMore }) {
  ...
  }<div className="absolute bottom-4 left-4 right-4 text-white"><div className="flex items-center gap-1 text-[#FFF8ED]/90 text-xs font-semibold uppercase tracking-wider mb-0.5"><MapPin className="w-3.5 h-3.5 text-brand-accent" />{story.state}</div><h3 className="font-serif text-xl sm:text-2xl font-bold tracking-tight text-white group-hover:text-brand-accent transition-colors duration-200">{story.city}</h3></div></div>{
    /* Card Content */
  }<div className="p-6 flex-grow flex flex-col justify-between" id={`story-body-${story.id}`}><div className="space-y-3">{
    /* Mood Badge */
  }<div className="flex items-center gap-1.5 text-xs text-brand-accent-dark font-semibold bg-brand-accent/10 px-2.5 py-1 rounded-md w-fit"><Heart className="w-3 h-3 fill-current" /><span>Mood: {story.mood}</span></div><p className="text-brand-text/80 text-sm leading-relaxed line-clamp-3">{story.shortDescription}</p></div>{
    /* Bottom Metadata & Button */
  }<div className="pt-5 mt-5 border-t border-brand-border/40 flex items-center justify-between"><div className="text-xs text-brand-text/60"><span className="font-semibold text-brand-primary block">{story.source === "locationNote" ? "Memory Type" : "Best Food"}</span><span className="line-clamp-1">{story.bestFood}</span></div><button
