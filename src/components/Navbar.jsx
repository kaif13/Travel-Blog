...
function Navbar({ activeTab, setActiveTab }) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      window.requestAnimationFrame(() => {
        setScrolled((current) => {
          const shouldBeScrolled = current ? window.scrollY > 8 : window.scrollY > 40;
  ...
  return <nav
    ...
    id="main-navbar"
  ><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center"><div className="flex items-center justify-between w-full">{
    /* Logo / Brand */
  }<div
    onClick={() => handleNavClick("home")}
    className="flex items-center gap-2 cursor-pointer group"
    id="nav-logo"
  ><div className="w-10 h-10 bg-brand-primary rounded-full flex items-center justify-center text-[#FFF8ED] transition-transform duration-300 group-hover:rotate-12"><Compass className="w-5 h-5 text-brand-accent" /></div><div><span className="font-serif text-xl sm:text-2xl font-bold text-brand-primary tracking-tight block">
                Kaif<span className="text-brand-accent">Travel</span>Blog
              </span><span className="text-[10px] uppercase tracking-widest text-brand-accent-dark font-medium -mt-1 block flex items-center gap-1">
                One Dream <Heart className="w-2.5 h-2.5 fill-current" /> Every State
              </span></div></div>{
    /* Desktop Navigation */
  }<div className="hidden md:flex items-center space-x-1" id="nav-desktop-menu">{navItems.map((item) => {
    const isActive = activeTab === item.id || item.id === "stories" && activeTab.startsWith("story-");
    return <button
      key={item.id}
      ...
      id={`nav-item-${item.id}`}
    >{item.label}{isActive && <motion.div
  ...
  >{isOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}</button></div></div></div>{
  ...
  ><div className="px-2 pt-2 pb-4 space-y-1 sm:px-3">{navItems.map((item) => {
    const isActive = activeTab === item.id || item.id === "stories" && activeTab.startsWith("story-");
    return <button
      key={item.id}
      onClick={() => handleNavClick(item.id)}
    ...
    >{item.label}</button>;
  })}</div></motion.div>}</AnimatePresence></nav>;
}
export {
  Navbar as default
};
